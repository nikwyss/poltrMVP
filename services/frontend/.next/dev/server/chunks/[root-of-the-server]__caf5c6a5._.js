...
"[project]/src/app/api/auth/verify-login/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    ...
    // Device binding: forward the initiator cookie so the appview only completes
    // the login in the browser that started it.
    ...
    const response = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(rest);
    if (session_token) {
        // Derive cookie maxAge from appview's expires_at (single source of truth)
        const maxAge = expires_at ? Math.floor((new Date(expires_at).getTime() - Date.now()) / 1000) : 7 * 24 * 60 * 60;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__caf5c6a5._.js.map