...
export default function BallotDetail() {
  ...
  const communityArgs = arguments_.filter(
    (a) => a.record.source?.$type !== "app.ch.poltr.ballot.argument#sourceOfficial",
  );
  const proArgs = communityArgs.filter((a) => a.record.type === "PRO");
  const contraArgs = communityArgs.filter((a) => a.record.type === "CONTRA");

  // Hero ratio bar reflects all arguments (community + official), not just
  // what's shown in the columns below.
  const allPro = arguments_.filter((a) => a.record.type === "PRO").length;
  const allContra = arguments_.filter((a) => a.record.type === "CONTRA").length;
  const totalArgs = allPro + allContra;
  const proPercent =
    totalArgs > 0 ? Math.round((allPro / totalArgs) * 100) : 50;
    ...
    >
      ...
      <nav className="flex items-center gap-2 pt-5 text-xs label">
        ...
        <div className="ml-auto">
          <ViewToggle active="community" ballotId={id} />
        </div>
      </nav>
