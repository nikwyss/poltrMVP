...
def delete_record(token: str, repo: str, collection: str, rkey: str) -> bool:
    ...
    if resp.status_code >= 400:
        print(
            f"    delete failed {collection}/{rkey}: {resp.status_code} {resp.text[:120]}"
        )
        return False
    return True
...
# ---------------------------------------------------------------------------
def reset_pds(
    did: str, token: str, community_crosspost_rkeys: set, execute: bool
) -> None:
    ...
    print(
        f"  arguments: {len(args)} total → keep {official} official, "
    ...
    print(
        f"  community crossposts (app.bsky.feed.post): delete {len(community_crosspost_rkeys)}"
    )
    if execute:
...
def reset_postgres(
    db_url: str, ballot_rkey: str, community_did: str, execute: bool
) -> set:
    ...
    try:
        with conn.cursor() as cur:
            ...
            steps = [
                (
                    "app_likes (ratings/likes)",
                    f"DELETE FROM app_likes WHERE {like_where}",
                ),
                (
                    "app_activity_seen",
                    f"DELETE FROM app_activity_seen WHERE {seen_where}",
                ),
                (
                    "app_comment_translations",
                    "DELETE FROM app_comment_translations WHERE ballot_rkey = %(b)s",
                ),
                ("app_comments", "DELETE FROM app_comments WHERE ballot_rkey = %(b)s"),
                (
                    "app_peerreview_responses",
                    f"DELETE FROM app_peerreview_responses WHERE {pr_where}",
                ),
                (
                    "app_peerreview_invitations",
                    f"DELETE FROM app_peerreview_invitations WHERE {pr_where}",
                ),
                (
                    "app_taxonomy_membership",
                    "DELETE FROM app_taxonomy_membership WHERE ballot_rkey = %(b)s",
                ),
                (
                    "app_taxonomy_node",
                    "DELETE FROM app_taxonomy_node WHERE ballot_rkey = %(b)s",
                ),
                (
            ...
            for label, stmt in steps:
                count_sql = (
                    "SELECT count(*) FROM ("
                    + stmt.replace("DELETE FROM", "SELECT 1 FROM", 1)
                    + ") s"
                )
...
def main():
    ...
    print(
        f"Reset ballot template — ballot_rkey={ballot_rkey} — PDS={PDS_HOST} — {mode}"
    )
