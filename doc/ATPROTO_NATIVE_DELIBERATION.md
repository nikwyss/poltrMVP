...
Peerreview-Responses, Invitations, Crossposts, Übersetzungen) ins **Per-Ballot-Governance-Repo** und hält
dafür die Governance-Credentials (`auth.governance_accounts`, entschlüsselt mit dem Master-Key). Damit ist
der am stärksten exponierte Dienst Träger der mächtigsten Schreib-/Credential-Fähigkeit.

**Ziel:** Die appview verliert den Zugriff aufs Governance-Repo. **User-authored** Inhalte werden
...
und schreibt den kanonischen **Community-Record** ins Governance-Repo.

## Architektur

```mermaid
flowchart TB
  FE["Frontend<br/>XRPC-Proxy"] --> AV["appview<br/>Gate synchron + autoritativ<br/>User-Creds · KEINE Gov-Creds"]
  ...
  subgraph INT["interne Schreib-Seite (vorläufig EIN Pod)"]
    ...
    WR["Writer (governance-writer)<br/>Gov-Creds + Master-Key<br/>Gate → Copy + source(uri,cid)"]
  end

  FH --> PJ
  PJ -->|stage| Q
  Q -->|LISTEN/NOTIFY + FOR UPDATE SKIP LOCKED| WR
  WR --> GR["Governance-Repo<br/>Community-Record (kanonisch)"]
...
2. **Community-Record = Copy mit Herkunfts-Referenz.** Der Governance-Record trägt den **vollen Inhalt**
   ...
   *Community-Akzeptanz* (Governance-Repo, gov-signiert, per CID an das Original gepinnt).

## Rollen-Arbeitsteilung

| | **Projektor** (Indexer, Node) | **Writer** (governance-writer, Python) |
|---|---|---|
| Hält | kein Gov-Key, keine `pw`-Spalten, keine PDS-Writes | **Gov-Key + Gov-Creds** + PDS-Sessions |
| Firehose | abonniert + projiziert (einziger Consumer) | **kein** Firehose-Client — pollt die DB-Work-Queue |
| PDS | nur lesen (Firehose) | **schreibt** ins Governance-Repo |
...
> (`atproto/governance.py`, `atproto/crosspost.py`, `atproto/acceptance.py`). Die **Naht**
...
3. **Projektor** sieht `collection=ballot.argument ∧ action=create ∧ !isGovernanceDid(did)` → stellt eine
...
4. **Writer** wird via `LISTEN/NOTIFY` geweckt, claimed die Zeile (`FOR UPDATE SKIP LOCKED`), gated
   ...
   unter der Governance-DID — mit **deterministischem create-only rkey** (idempotent, crash-recovery).
5. Der governance-signierte Community-Record kommt über den **Firehose zurück** zum Projektor.
...
| Pfad | Input (User-Repo) | Writer | Output (Governance-Repo) |
|------|-------------------|--------|--------------------------|
...
der Gov-Record kommt über den Firehose zurück. (Diese *interne* Queue widerspricht „kein Kanal" nicht — das
galt für *appview*.)
...
- Gov-Creds + Master-Key liegen auf der **internen** Schreib-Seite, nicht mehr in der internet-zugewandten
  appview. appview behält den Master-Key nur noch für **User**-App-Passwörter (`auth_creds`) → Empfehlung:
  Master-Key in zwei separate Keys splitten (User-Key in appview, Gov-Key in Writer/CMS); siehe `doc/TODO.md`.
...
- **Drift gelöst:** die interne Seite reagiert nur auf **Create**-Events; der Community-Record gehört dem
  Governance-Konto → vom Autor nicht editier-/zurückziehbar (Retraktion nur über Moderation/Ozone).
...
| Flag | Dienst | Wirkung |
|------|--------|---------|
| `APPVIEW_ARGS_USER_REPO_ENABLED` | appview | Argument-Create → User-Repo statt Governance |
| `APPVIEW_RESPONSES_USER_REPO_ENABLED` | appview | PR-Response → Reviewer-Repo statt Governance |
