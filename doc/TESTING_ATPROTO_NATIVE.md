...
**`app_acceptance_queue` (Postgres)** → **Writer (Python)** → **Governance-Repo** → zurück. Es gibt
...
- `_accept_argument`: Happy-Path schreibt Community-Record mit `source.originUri/originCid` + korrektem rkey;
  ineligible → `rejected/not_eligible`; kein Gov-Konto → `rejected`; **Idempotenz** (existierender Record →
  `done`, kein zweiter Write).
- `_accept_response`: Happy-Path (gov-DID aus `app_arguments`, top-level `originUri/originCid`,
...
- `argument.create` mit Flag **on** → schreibt ins **User-Repo** (`pds_create_record`); Flag **off** →
  **Governance** (`create_governance_record`). (Deps gemockt, Rate-Limiter im Test deaktiviert.)
...
1. **Argument anlegen** → `goat record get` zeigt es im **User-Repo** (user-signiert); kurz darauf existiert
   der **Community-Record** im Governance-Repo mit `source.originUri/originCid`; `app_arguments`-Zeile mit
   `origin_uri/origin_cid`; `app_acceptance_queue`-Zeile `done`.
2. **PR-Response** analog → Community-Response im Gov-Repo (rkey wie Legacy), `app_peerreview_responses`
